...
<div class="container-fluid">
 <h1 class="h3 mb-2 text-gray-800">Subject Table</h1>
...
$count_result = $conn->query("SELECT COUNT(*) AS total FROM subject WHERE subject_code LIKE '$like' OR des LIKE '$like'");
$total_rows   = $count_result->fetch_assoc()['total'];
$total_pages  = max(1, ceil($total_rows / $per_page));
if ($page > $total_pages) $page = $total_pages;

$result = $conn->query("SELECT * FROM subject WHERE subject_code LIKE '$like' OR des LIKE '$like' ORDER BY id DESC LIMIT $per_page OFFSET $offset");
?>
    ...
    <div class="card-header py-3">
        <div class="d-flex justify-content-between align-items-center flex-wrap" style="gap:8px;">
            ...
            <div class="d-flex align-items-center flex-wrap" style="gap:8px;">
                <form method="GET" action="" class="mb-0" id="searchForm">
                    <div class="input-group input-group-sm" style="width:260px;">
                        ...
                        <input type="text" class="form-control border-left-0" name="search" id="liveSearch"
                               placeholder="Search subjects..." value="<?= htmlspecialchars($search); ?>"
                               autocomplete="off" style="border-radius:0 4px 4px 0;">
                        <?php if($search): ?>
                        <div class="input-group-append">
                            <a href="subject.php" class="btn btn-outline-secondary btn-sm" id="clearSearch" title="Clear search">
                ...
                <a href="#" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#addSubjectModal">
    ...
    <form action="code/subject.php" method="POST">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="form-group"><label>Subject Code:</label><input type="text" class="form-control" name="subject" required></div>
          <div class="form-group"><label>Description:</label><input type="text" class="form-control" name="description" required></div>
          <div class="form-group"><label>Units:</label><input type="number" class="form-control" name="units" required></div>
        </div>
  ...
  <div class="modal-dialog" role="document">
    <form action="code/subject.php" method="POST">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="form-group"><label>Subject Code:</label><input type="text" class="form-control" id="edit-subject" name="edit-subject" required></div>
          <div class="form-group"><label>Description:</label><input type="text" class="form-control" id="edit-des" name="edit-des" required></div>
          <div class="form-group"><label>Units:</label><input type="number" class="form-control" id="edit-unit" name="edit-unit" required></div>
        </div>
        <div class="modal-footer">
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="#" class="btn btn-sm btn-primary editBtn"
                                   ...
                                   data-des="<?= htmlspecialchars($row['des']); ?>"
                                   data-unit="<?= htmlspecialchars($row['unit']); ?>">Edit</a>
                                <a href="delete.php?type=subject&id=<?= $row['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure?')">Delete</a>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="4" class="text-center text-muted py-3">No subject found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
...
$(document).ready(function () {
  $('.editBtn').on('click', function () {
    ...
    $('#edit-des').val($(this).data('des'));
    $('#edit-unit').val($(this).data('unit'));
  });
});
</script>
